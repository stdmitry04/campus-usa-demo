...
class SecureS3Storage(S3Boto3Storage):
	...
	def _validate_file_security(self, content):
		...
		if hasattr(content, 'size') and content.size > max_size:
			raise ValidationError(
				f'file size ({content.size / 1024 / 1024:.1f}MB) exceeds maximum '
				f'allowed size ({max_size / 1024 / 1024:.1f}MB)'
			)

		# file extension validation
		if hasattr(content, 'name'):
			self._validate_file_extension(content.name)

		# mime type validation
		self._validate_file_mimetype(content)

	def _validate_file_extension(self, filename):
		"""validate file extension matches allowed list"""
		allowed_extensions = getattr(settings, 'ALLOWED_DOCUMENT_EXTENSIONS', ['.pdf'])
	...
	def _validate_file_mimetype(self, content):
		...
		try:
			...
			content.seek(0)
			file_content = content.read(1024)
			content.seek(0)
			...
			if kind is None:
				# couldn't determine file type, check if it might be plain text
				if hasattr(content, 'name') and content.name.lower().endswith('.txt'):
					mime_type = 'text/plain'
				else:
					raise ValidationError('could not determine file type for security validation')
			else:
				mime_type = kind.mime
	...
	def _generate_secure_filename(self, original_name):
		...
		# generate secure path: documents/user_id/year/month/uuid_filename.ext
